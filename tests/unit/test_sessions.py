"""The sessions use case summarises live interactive sessions in chooser order, token-free."""
...
def test_summaries_describe_each_live_session_and_whether_it_is_registered() -> None:
    ...
    assert list_sessions(deps) == (
        SessionSummary(Pid(20), SessionStatus.IDLE, "proj", "/home/me/proj", registered=False),
        SessionSummary(Pid(10), SessionStatus.IDLE, "alpha", "/work", registered=True),
    )
...
def test_background_sessions_are_left_out() -> None:
    deps = _deps(session(10, kind=SessionKind.BACKGROUND), session(20))
    assert [s.pid for s in list_sessions(deps)] == [Pid(20)]
