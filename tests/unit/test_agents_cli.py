"""AgentsCli parses ``claude agents --json`` and keeps live interactive sessions."""
...
from explain_selection.domain import SessionStatus
...
def test_lists_only_live_interactive_sessions() -> None:
    runner = _runner([INTERACTIVE, BACKGROUND, DEAD])
    sessions = AgentsCli(runner).list_interactive()
    assert [s.pid for s in sessions] == [4242]
    only = sessions[0]
    assert only.cwd == "/work/proj"
    assert only.status is SessionStatus.IDLE
    assert only.session_id == "sid-1"
    assert only.started_at_ms == 1_700_000_000_000
...
def test_missing_status_is_treated_as_busy() -> None:
    ...
    sessions = AgentsCli(_runner([payload])).list_interactive()
    assert sessions[0].status is SessionStatus.BUSY
...
def test_non_zero_exit_raises() -> None:
    ...
    with pytest.raises(AgentsQueryError):
        AgentsCli(runner).list_interactive()
...
def test_the_command_is_the_agents_json_call() -> None:
    runner = _runner([])
    AgentsCli(runner, claude_bin="/opt/claude").list_interactive()
